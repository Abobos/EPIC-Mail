...
function validateUserSignUpDetails(userDetails) {
  var schema = {
    firstName: _joi.default.string().regex(/^[a-zA-Z]+$/).required().error(new Error('firstName is not valid')),
    lastName: _joi.default.string().regex(/^[a-zA-Z]+$/).required().error(new Error('lastName is not valid')),
    email: _joi.default.string().trim().email().regex(/^[A-Za-z0-9_]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$/).required().error(new Error('email is not valid')),
...
function validateUserSignInDetails(userDetails) {
  var schema = {
    email: _joi.default.string().email().regex(/^[A-Za-z0-9_]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$/).required().error(new Error('email is not valid')),
